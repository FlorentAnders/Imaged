package com.fa.imaged.adapters;
...
import com.fa.imaged.activity.DetailActivityV2;
import com.fa.imaged.activity.EditProfileActivity;
import com.fa.imaged.activity.FollowersActivity;
import com.fa.imaged.activity.FollowingActivity;
import com.fa.imaged.activity.HomeActivity;
import com.fa.imaged.activity.ProfileActivity.EditRelationShip;
import com.fa.imaged.R;
import com.fa.imaged.image.PicassoRound;
import com.fa.imaged.posts.InstagramImage;
import com.fa.imaged.utils.BlurTransform;
import com.fa.imaged.views.LinearLayoutWebBackground;
...
public class ProfileImagesAdapter extends RecyclerView.Adapter<ProfileImagesAdapter.ProfileHolder> {
	...
	public static class ProfileHolder extends ViewHolder implements View.OnClickListener {
			...
			public FancyButton profileBtn;
            public LinearLayoutWebBackground profileImage;
	        public ProfileHolder(View itemView, int itemType) {
	            ...
	            if(itemType == TYPE_ITEM) {
	    	        ...
	    	        profileBtn = (FancyButton) itemView.findViewById(R.id.profile_btn);
                    ...
                    profileImage = (LinearLayoutWebBackground) itemView.findViewById(R.id.profile_background);
	...
	public void onBindViewHolder(ProfileHolder holder, int i) {
       if(holder.Holderid ==1) {                              // as the list view is going to be called after the header view so we decrement the
    	    ...
    	    Picasso.with(holder.itemView.getContext()).load(image.thumbnail).placeholder(android.R.color.background_light).into(holder.gridThumbnail);
    	   	...
    	   	holder.profileBtn.setBackgroundColor(holder.profile.getContext().getResources().getColor(android.R.color.transparent));
            ...
            Picasso.with(holder.profile.getContext()).load(string_profilepic).resize(60, 60).centerCrop().placeholder(R.color.colorPrimary).transform(new BlurTransform(holder.profile.getContext(), 10f, true)).into(holder.profileImage);
            holder.profileBtn.setText(string_profile_btn);
            if(string_profile_btn == activity.getResources().getString(R.string.edit_profile)) {
